...
| MS9 | Arrangement of the time panels: 1–2 rows are stacked with the maps in a left column; more rows need `grid` (rows × columns of condition keys, each once, e.g. a 2 × 3 design as 2 rows of 3) — the script stops without it. A grid with more than one column puts the template maps in one row above it; y labels once per grid row. (User rule, 2026-09-26: never stack six conditions.) | U | Code |
