...
| MS7 | (a) Under each map its longest run per row (`—` if absent), while there are ≤ 4 rows; (b) ribbon segments read `S1`, `S2`, … like the maps, in white on dark/saturated states and black on light ones (relative luminance > 0.3); (c) the GFP trace is labelled "GFP"; (d) panel titles carry the condition name only, never N (n goes in the caption facts). Maps carry no electrode marks. (User rules, 2026-09-26.) | U | Code |
