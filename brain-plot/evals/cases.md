...
|---|---|---|---|
| 9 | combo figure, two groups, window from the user | interview rounds; written spec confirmed before `plot`; PNG checked; `qa` filled in `_run.json` | invented `time_locked_to`/`reference`; asks the user for `time_locked_to`, `reference` or the display range; no question on the claim or key comparison; statistics in the caption that the user did not give |
